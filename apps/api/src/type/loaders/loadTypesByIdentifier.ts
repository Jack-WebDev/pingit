 ...
 */
export function loadTypesByIdentifier(
    ctx: Context
): DataLoader<string, Type | null> {
    return new DataLoader(async (identifiers: readonly TypeIdentifier[]) => {
        const results = await Type.query(ctx.db).whereIn(
            'identifier',
            identifiers
        );
        ...
        return identifiers.map(
            (identifier) =>
                results.find((o) => o.identifier === identifier) || null
        );
    });
}
