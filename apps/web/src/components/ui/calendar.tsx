...
function CalendarChevron({
	...
	// Pick an icon; if you don't have ChevronUpIcon, rotate Down.
	const Icon =
		orientation === "left"
			? ChevronLeftIcon
			: orientation === "right"
	...
	const rotateUp = orientation === "up" ? "rotate-180" : undefined;

	return (
		<Icon
			// apply width/height from size; keep your tailwind size as fallback if you want
			width={size}
			height={size}
			className={cn("size-4", disabled && "opacity-50", rotateUp, className)}
			aria-hidden
			focusable="false"
		/>
	);
